...
import './FRApp.css';
import { mcqQuestions as frMcqQuestions, longAnswerQuestions as frLongAnswerQuestions } from '../data/frQuestions.js';
...
const Header = ({ currentSection, onNavigate }) => {
  ...
  return (
    <header className="app-header fr-header">
...
const Dashboard = ({ onStartPractice }) => {
  const topics = [
    { name: "Consolidation", mcqs: 1, longAnswers: 1, progress: 45 },
    { name: "Financial Instruments", mcqs: 1, longAnswers: 0, progress: 30 },
    { name: "Revenue Recognition", mcqs: 1, longAnswers: 1, progress: 60 },
    { name: "Leases", mcqs: 1, longAnswers: 0, progress: 40 },
    { name: "Employee Benefits", mcqs: 1, longAnswers: 0, progress: 25 },
    { name: "Ind AS 101", mcqs: 1, longAnswers: 0, progress: 20 },
    { name: "Impairment", mcqs: 1, longAnswers: 0, progress: 35 },
    { name: "Business Combinations", mcqs: 1, longAnswers: 0, progress: 15 },
  ];

  return (
    <div className="dashboard fr-dashboard">
      <div className="dashboard-hero">
        <div className="hero-content">
          <h2>Welcome Back!</h2>
          <p>Continue your preparation for CA Final Paper 1: Financial Reporting</p>
          <div className="hero-stats">
            <div className="stat-card">
              <div className="stat-value">{frMcqQuestions.length}</div>
        ...
        <div className="hero-visual">
          <div className="circular-progress-large">
            <svg viewBox="0 0 100 100">
              ...
              <circle cx="50" cy="50" r="45" fill="none" stroke="url(#fr-gradient)" strokeWidth="8" 
                strokeDasharray="283" strokeDashoffset="170" strokeLinecap="round"
                transform="rotate(-90 50 50)"/>
              <defs>
                <linearGradient id="fr-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
        ...
        <div className="action-cards">
          <button className="action-card mcq-card" onClick={() => onStartPractice('mcq')}>
            ...
            <div className="action-content">
              <h4>MCQ Practice</h4>
              <p>{frMcqQuestions.length} questions from past papers</p>
            </div>
          ...
          <button className="action-card long-card" onClick={() => onStartPractice('long')}>
            ...
            <div className="action-content">
              <h4>Long Answers</h4>
              <p>{frLongAnswerQuestions.length} comprehensive problems</p>
            </div>
            <div className="action-arrow">→</div>
          </button>
          ...
          {topics.map((topic, index) => (
            ...
            >
              ...
              <div className="topic-meta">
                ...
                <span>{topic.longAnswers} Long Answers</span>
              </div>
            </button>
          ))}
        </div>
...
const MCQPractice = () => {
  ...
  const currentQuestion = frMcqQuestions[currentQuestionIndex];

  // Reset context expanded state when question changes
  useEffect(() => {
  ...
  const handleNext = () => {
    if (currentQuestionIndex < frMcqQuestions.length - 1) {
      setCurrentQuestionIndex(prev => prev + 1);
      setSelectedAnswer(answers[frMcqQuestions[currentQuestionIndex + 1].id] ?? null);
      setIsSubmitted(!!answers[frMcqQuestions[currentQuestionIndex + 1].id]);
  ...
  const handlePaletteClick = (index) => {
    ...
    setSelectedAnswer(answers[frMcqQuestions[index].id] ?? null);
    setIsSubmitted(!!answers[frMcqQuestions[index].id]);
    setShowExplanation(false);
  };
  ...
  return (
    <div className="mcq-practice fr-mcq">
      <div className="practice-header">
        <div className="question-nav">
          ...
          <span className="question-counter">
            Question {currentQuestionIndex + 1} of {frMcqQuestions.length}
          </span>
          <button 
            className="nav-arrow" 
            onClick={handleNext}
            disabled={currentQuestionIndex === frMcqQuestions.length - 1}
          >
            →
          </button>
        </div>
      ...
      <div className="question-container">
        ...
        {currentQuestion.caseContext && (
          <div className="case-context-panel">
            <div className="case-context-header" onClick={toggleContext}>
              <span>📋 Case Study Context</span>
              <span className="toggle-icon">{isContextExpanded ? '▼' : '▶'}</span>
            </div>
            {isContextExpanded && (
              <div className="case-context-body">
                <pre>{currentQuestion.caseContext}</pre>
              </div>
            )}
          </div>
        )}
        ...
        {currentQuestion.caseScenario && (
          <div className="case-scenario">
            <h4>Case Scenario</h4>
            <pre>{currentQuestion.caseScenario}</pre>
          </div>
        )}
        
        {/* Ind AS Reference Badge */}
        {currentQuestion.indAsReference && (
          <div className="ind-as-reference-badge">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/>
              <polyline points="14,2 14,8 20,8"/>
            </svg>
            <span>Ind AS Reference: {currentQuestion.indAsReference}</span>
          </div>
        )}
        ...
        <div className="question-meta">
          ...
          {currentQuestion.indAsReference && (
            <span className="ind-as-badge-inline">{currentQuestion.indAsReference}</span>
          )}
...
const LongAnswerPractice = () => {
  ...
  const currentQuestion = frLongAnswerQuestions[currentQuestionIndex];
  ...
  const handleNext = () => {
    if (currentQuestionIndex < frLongAnswerQuestions.length - 1) {
      setCurrentQuestionIndex(prev => prev + 1);
      setUserAnswer('');
  ...
  return (
    <div className="long-answer-practice fr-long">
      <div className="practice-header">
        <div className="question-nav">
          <button 
            className="nav-arrow" 
          ...
          <span className="question-counter">
            Question {currentQuestionIndex + 1} of {frLongAnswerQuestions.length}
          </span>
          <button 
            className="nav-arrow" 
            onClick={handleNext}
            disabled={currentQuestionIndex === frLongAnswerQuestions.length - 1}
          >
            →
          </button>
        </div>
...
• Include relevant calculations"
          ...
          <>
            <div className="model-answer-section">
              ...
              {currentQuestion.workingNotes && (
                <div className="working-notes-section">
                  <button 
                    className="working-notes-toggle"
                  ...
                  {showWorkingNotes && (
                    <div className="working-notes-content">
      ...
      <div className="case-study-palette">
        ...
        <div className="palette-list">
          {frLongAnswerQuestions.map((q, index) => (
            <button
              key={q.id}
