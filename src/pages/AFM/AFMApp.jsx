...
const Dashboard = ({ onStartPractice }) => {
  const topics = [
    { name: "Security Valuation & CAPM", mcqs: 5, longAnswers: 0, progress: 45 },
    { name: "Portfolio Management", mcqs: 1, longAnswers: 1, progress: 60 },
    { name: "Mutual Funds", mcqs: 3, longAnswers: 0, progress: 30 },
    { name: "Derivatives & Options", mcqs: 5, longAnswers: 0, progress: 55 },
    { name: "Foreign Exchange", mcqs: 4, longAnswers: 0, progress: 40 },
    { name: "Mergers & Acquisitions", mcqs: 1, longAnswers: 0, progress: 25 },
    { name: "Capital Budgeting", mcqs: 0, longAnswers: 1, progress: 20 },
    { name: "Interest Rate Risk", mcqs: 1, longAnswers: 1, progress: 15 },
  ];

  return (
    <div className="dashboard afm-dashboard">
      <div className="dashboard-hero">
        <div className="hero-content">
          <h2>Welcome Back!</h2>
          <p>Continue your preparation for CA Final Paper 2: Advanced Financial Management</p>
          <div className="hero-stats">
            <div className="stat-card">
              <div className="stat-value">{afmMcqQuestions.length}</div>
        ...
        <div className="hero-visual">
          <div className="circular-progress-large">
            <svg viewBox="0 0 100 100">
              ...
              <circle cx="50" cy="50" r="45" fill="none" stroke="url(#afm-gradient)" strokeWidth="8" 
                strokeDasharray="283" strokeDashoffset="170" strokeLinecap="round"
                transform="rotate(-90 50 50)"/>
              <defs>
                <linearGradient id="afm-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
        ...
        <div className="action-cards">
          <button className="action-card mcq-card" onClick={() => onStartPractice('mcq')}>
            ...
            <div className="action-content">
              <h4>MCQ Practice</h4>
              <p>{afmMcqQuestions.length} questions from past papers</p>
            </div>
          ...
          <button className="action-card long-card" onClick={() => onStartPractice('long')}>
            ...
            <div className="action-content">
              <h4>Case Studies</h4>
              <p>{afmLongAnswerQuestions.length} comprehensive problems</p>
            </div>
            <div className="action-arrow">→</div>
          </button>
...
const MCQPractice = () => {
  ...
  const currentQuestion = afmMcqQuestions[currentQuestionIndex];
  ...
  const handleNext = () => {
    if (currentQuestionIndex < afmMcqQuestions.length - 1) {
      setCurrentQuestionIndex(prev => prev + 1);
      setSelectedAnswer(answers[afmMcqQuestions[currentQuestionIndex + 1].id] ?? null);
      setIsSubmitted(!!answers[afmMcqQuestions[currentQuestionIndex + 1].id]);
  ...
  return (
    <div className="mcq-practice afm-mcq">
      <div className="practice-header">
        <div className="question-nav">
          ...
          <span className="question-counter">
            Question {currentQuestionIndex + 1} of {afmMcqQuestions.length}
          </span>
          <button 
            className="nav-arrow" 
            onClick={handleNext}
            disabled={currentQuestionIndex === afmMcqQuestions.length - 1}
          >
            →
          </button>
        </div>
...
const LongAnswerPractice = () => {
  ...
  return (
    <div className="long-answer-practice afm-long">
      <div className="practice-header">
        <div className="question-nav">
          <button 
            className="nav-arrow" 
          ...
          <span className="question-counter">
            Case Study {currentQuestionIndex + 1} of {afmLongAnswerQuestions.length}
          </span>
          <button 
            className="nav-arrow" 
            onClick={handleNext}
            disabled={currentQuestionIndex === afmLongAnswerQuestions.length - 1}
          >
            →
          </button>
        </div>
...
• Include relevant calculations"
      ...
      <div className="case-study-palette">
        ...
        <div className="palette-list">
          {afmLongAnswerQuestions.map((q, index) => (
            <button
              key={q.id}
